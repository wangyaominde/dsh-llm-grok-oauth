window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const en = {
			...
			logout: "Sign out",
			hint: "If the official Grok CLI is already signed in, that session is reused. Otherwise a browser sign-in page opens. If https://auth.x.ai is unreachable, run grok login in a terminal, then click Sign in again.",
		...
		const zh = {
			login: "使用 Grok 账号登录",
			logout: "退出登录",
			hint: "已登录官方 Grok CLI 时将直接复用该会话；否则打开 xAI 授权页。无法访问 auth.x.ai 时，请先在终端执行 grok login，再点击登录。",
		...
		function findGrokRow() {
			...
			for (const button of buttons) {
				...
				const row = button.closest("li") || button.closest("[class*='rowCard']") || button.closest("[class*='setupCard']");
				if (row) return row;
			}
			const items = document.querySelectorAll("li, [class*='rowCard'], [class*='setupCard']");
		...
		function apply(ctx) {
			...
			scope.subscribe(() => {
				...
				if (cur.oauthStatus === "pending" && next.oauthStatus === "signed-out" && !next.userCode) {
			...
			const applyRemote = (body) => {
				...
				store.set({
					...store.getSnapshot(),
					oauthStatus: body.oauthStatus || store.getSnapshot().oauthStatus,
					verificationUrl: body.verificationUrl || "",
					userCode: body.userCode || "",
					oauthMessage: body.oauthMessage || "",
				});
			};
