 ...
 * Settings → Models → Grok (xAI 订阅) (no separate page). The live account
...
export function apply(ctx, config) {
  ...
  const consumeAction = async () => {
    ...
    try {
      ...
      if (action === 'login') {
        if (oauth.shouldIgnoreStaleLogin()) return;
  ...
  ctx.inject(['webServer'], (webCtx) => {
    ...
    webCtx.effect(() => webCtx.webServer.register({
      ...
      handler: handle('GET', async () => {
        const live = await oauth.uiStatus();
        if (live !== undefined) lastStatus = { ...lastStatus, ...live };
      }),
    ...
    webCtx.effect(() => webCtx.webServer.register({
      kind: 'exact',
      path: '/api/llm-grok/cancel',
      handler: handle('POST', async () => {
        await oauth.cancelLogin();
      }),
    }), 'llm-grok: cancel route');
  });
